...
public:
  ...
  void init(ProcessMessage* procMessage, int *arrPins, int iNumPins, bool bEMatchMode);
  void loop();
...
private:
  ...
  ProcessMessage*   m_procMessage;

  bool              m_bCyclingRelays;
  unsigned long     m_tNextCycle;
  uint8_t           m_u8CycleIndex;
};
