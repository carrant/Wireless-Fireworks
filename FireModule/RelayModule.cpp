...
void RelayModule::init(ProcessMessage* procMessage, int *arrPins, int iNumPins, bool bEMatchMode)
{
  m_procMessage     = procMessage;
  m_lsActiveRelays  = NULL;
...
void RelayModule::loop()
{
  ...
  if(m_bCyclingRelays)
  {
    if (millis() > m_tNextCycle)
    {
      relayOn(m_u8CycleIndex++);
      relayOn(m_u8CycleIndex++);
  
      if (m_u8CycleIndex >= m_numRelays)
        m_bCyclingRelays = false;
      else
        m_tNextCycle = millis() + m_tRelayOnDuration + 100;
    }
  }
...
void RelayModule::checkRelay()
{
  ...
  while(pCur)
  {
    if ((tTime - pCur->tStartTime) > m_tRelayOnDuration)
    {
      ...
      if (m_procMessage)
        m_procMessage->sendFired(arridRelayFired, 1);
...
void RelayModule::cycleAllRelays()
{
  if (m_bCyclingRelays)
    return; // already cycling
    m_u8CycleIndex = 0;
    
  m_bCyclingRelays = true;
  m_tNextCycle = millis() + m_tRelayOnDuration + 100;
